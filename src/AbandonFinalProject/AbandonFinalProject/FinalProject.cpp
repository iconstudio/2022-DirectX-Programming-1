...
void InitialzeGame(HWND hwnd)
{
	...
	gameRenderer.Start();
	gameRenderer.WaitForGpuComplete();

	// 작업 가능
	gameRenderer.ResetCmdAllocator();
	gameRenderer.ResetCmdList();
	...
	gameRenderer.CloseCmdList();
	P3DCommandList cmd_lists[] = { dxcmdlist };
	gameRenderer.ExecuteCmdList(cmd_lists, std::size(cmd_lists));
