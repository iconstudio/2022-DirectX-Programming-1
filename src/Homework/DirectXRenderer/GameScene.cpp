...
{
	Transformer::SetProjection(myCamera->projectionPerspective);
...
bool GameScene::CheckCameraBounds(const GameObject* obj) const
{
	return myCamera->IsInFrustum(obj->myCollider);
}
...
void GameScene::PrepareRenderingCollider(const BoundingOrientedBox& collider)
{
	...
	for (int i = 0; i <= 4; ++i)
	{
		...
		const auto& vtx_from = Transformer::ProjectTransform(place[j]);
		const auto& vtx_to = Transformer::ProjectTransform(place[j + 1]);
		const auto& vtx_far = Transformer::ProjectTransform(place[j + 4]);
...
void GameScene::CastParticles(XMFLOAT3&& pos, ParticleBlob* blob)
{
	...
	{
		auto& particle = blob->myParticles[i];
		particle->SetPosition(std::forward<XMFLOAT3>(pos));
