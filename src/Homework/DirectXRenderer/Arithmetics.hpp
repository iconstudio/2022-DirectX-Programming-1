...
namespace Vector3
{
	...
	inline float Distance(XMFLOAT3&& vector1, XMFLOAT3&& vector2)
	{
		XMFLOAT3 result{};

		const auto v1 = XMLoadFloat3(std::forward<XMFLOAT3*>(&vector1));
		const auto v2 = XMLoadFloat3(std::forward<XMFLOAT3*>(&vector2));
		const auto dist = XMVector3Length(XMVectorSubtract(v1, v2));
		XMStoreFloat3(&result, dist);

		return result.x;
	}

	inline float Distance(const XMFLOAT3& vector1, XMFLOAT3&& vector2)
	{
		return Distance(XMFLOAT3(vector1), std::forward<XMFLOAT3>(vector2));
	}

	inline float Distance(XMFLOAT3&& vector1, const XMFLOAT3& vector2)
	{
		return Distance(std::forward<XMFLOAT3>(vector1), XMFLOAT3(vector2));
	}

	inline float Distance(const XMFLOAT3& vector1, const XMFLOAT3& vector2)
	{
		return Distance(XMFLOAT3(vector1), XMFLOAT3(vector2));
	}
